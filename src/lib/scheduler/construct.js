...
export function constructSchedule({ mode, week, employees, absences, allWeeks, preferences, referenceWeek, trace }) {
  ...
  if (mode === "replan") {
    w.shifts.forEach((s) => {
      ...
      if (keepPos1 || keepPos3 || keepGeneral.length) {
        warnings.push({ code: WARNING.MANUAL_ASSIGNMENT_PRESERVED, shiftId: s.id, date: s.date, type: s.type });
      }
  ...
  // Pocita sa len z PRIMARNYCH rol (nie zaskokov) - zaskok nemeni, ktora rola
  // je "vzacnejsia".
  ...
  // "Zaskok" (backup) rola pre danu kritcku poziciu - pos1-backup existuje
  // oddavna, pos3-backup je analogicky mechanizmus na explicitne uprednostnenie
  // konkretnych ludi pre pozicu 3 (napr. "Achacova ma prednost pred Jechovou" -
  // nastavi sa cez rolu v zalozke Zamestnanci: primarny clovek = 'pos3',
  // ten s nizsou prioritou = 'pos3-backup', presne ako uz funguje pre hrncovu).
  const BACKUP_ROLE = { pos1: "pos1-backup", pos3: "pos3-backup" };
  ...
  roleOrder.forEach((role) => {
    w.shifts.forEach((shift) => {
      ...
      // Zaskok sa pouzije nielen ked je primarny pool prazdny, ale UZ AJ VTEDY,
      // ked by jediny zostavajuci primarny kandidat musel dostat vynimocnu 5.
      // zmenu - kym zaskok je stale k dispozicii pod svojim normalnym cielom.
      // Zaskok sa prida do SPOLOCNEHO poolu a rozhodne skore (scoring.js uz
      // penalizuje 5. zmenu vysoko), nie hardcodovana priorita - takze ak by aj
      // zaskok potreboval 5. zmenu, moze byt aj tak vybrany primarny clovek
      // (rovnaky "najmensie zlo" princip).
      ...
      if (!primaryHasCleanOption) {
        const backupCands = eligibleForRole(shift, BACKUP_ROLE[role], activeEmployees, absences, w);
      ...
      if (pick) {
        ...
        if (usedBackup) {
          const backupWarningCode = role === "pos1" ? WARNING.POS1_BACKUP_USED : WARNING.POS3_BACKUP_USED;
          warnings.push({ code: backupWarningCode, shiftId: shift.id, date: shift.date, type: shift.type, role, employeeId: pick.id });
        }
        if (weekShiftCount(w, pick.id) >= HARD_MAX_SHIFTS) {
          warnings.push({ code: WARNING.EXCEPTIONAL_FIFTH_SHIFT, shiftId: shift.id, date: shift.date, type: shift.type, role, employeeId: pick.id });
        }
      } else {
        warnings.push({ code: WARNING.CRITICAL_ROLE_SHORTAGE, shiftId: shift.id, date: shift.date, type: shift.type, role });
      }
    });
  });
  ...
  w.shifts.forEach((shift) => {
    ...
    if (already.length < needed) {
      const filled = shift.assigned.general.length + (shift.assigned.pos1 ? 1 : 0) + (shift.assigned.pos3 ? 1 : 0);
      warnings.push({ code: WARNING.STAFFING_SHORTAGE, shiftId: shift.id, date: shift.date, type: shift.type, filled, total });
    }
  });
